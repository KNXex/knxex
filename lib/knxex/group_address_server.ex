defmodule KNXex.GroupAddressServer do
  @moduledoc """
  ...
  defmacrop validate_opts_group_addresses(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      case opts[:group_addresses] do
        %{} = map ->
          Map.new(map, fn
            {_key, %KNXex.EtsProject.GroupAddressInfo{} = term} ->
              if term.type == nil do
            ...
            {key, type} when is_binary(key) and is_binary(type) ->
              case KNXex.GroupAddress.from_string(key) do
                {:ok, grpaddr} ->
                  {grpaddr,
                   %GroupAddressData{
                     address: grpaddr,
        ...
        nil ->
          ets_project =
            case opts[:ets_project] do
              path when is_binary(path) ->
                KNXex.ProjectParser.parse(path, only: [:group_addresses])

              %KNXex.EtsProject{} = term ->
                term
          ...
          |> Enum.filter(fn
            ...
            {_key, %KNXex.EtsProject.GroupAddressInfo{type: nil}} -> false
            {_key, %KNXex.EtsProject.GroupAddressInfo{}} -> true
            _any -> false
          end)
          |> Map.new(fn
            {_key, %KNXex.EtsProject.GroupAddressInfo{} = term} ->
    ...
    knx_interface =
      case opts[:knx_interface] do
        ...
        {module, name_or_pid} = val when is_atom(name_or_pid) or is_pid(name_or_pid) ->
          ...
          if module != KNXex.MulticastClient and
               module != KNXex.TunnelClient do
            raise ArgumentError,
    ...
    case :ets.whereis(__MODULE__) do
      ...
      _tid ->
        ...
        |> Enum.map(fn %GroupAddressData{} = grpaddr ->
          {KNXex.GroupAddress.to_string(grpaddr.address), grpaddr.value,
  ...
  def add(%KNXex.GroupAddress{} = group_address, dpt, opts \\ [])
  ...
  def remove(%KNXex.GroupAddress{} = group_address) do
  ...
  """
  @spec write(KNXex.GroupAddress.t(), term(), Keyword.t()) ::
          :ok | {:error, term()}
  def write(%KNXex.GroupAddress{} = group_address, value, opts \\ []) when is_list(opts) do
    GenServer.call(
      get_opts_name(),
  ...
  - `manufacturer_map: map()` - A map from `KNXex.ProjectParser.parse_manufacturers/2`, if it has already been done before.
      ...
      if mode == true do
        ...
        |> Enum.filter(fn
          {_key, %GroupAddressData{} = term} ->
            KNXex.GroupAddress.to_string(term.address) in mode
        end)
        |> Enum.into(%{})
      end
  ...
  @doc false
  def handle_info({:knx, %KNXex.Telegram{type: type} = telegram}, %State{} = state)
  ...
  defp hydrate_from_hydration_state({address, value, timestamp}, acc) do
    grpaddr = KNXex.GroupAddress.from_string(address)
  ...
  defp generate_hydration_gpas_list_from_ets(
    ...
    manufacturer_map =
      opts[:manufacturer_map] ||
        KNXex.ProjectParser.parse_manufacturers(
  ...
  for {_type, name, dpt} <- KNXex.DPT.get_dpts() do
