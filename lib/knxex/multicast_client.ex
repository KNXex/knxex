defmodule KNXex.MulticastClient do
  @moduledoc """
  ...
  - `frame_callback: (KNXex.Frame.t(), :handled | unhandled) -> any()` - Optional. A callback module can be specified,
  that will be called for all frames which are not explicitely handled by this server module.
  Two arity: (KNXex.Frame.t(), :handled | :unhandled) - :handled for frames handled by this server module.
  ...
  - `source_address: KNXex.IndividualAddress.t()` - Required. The KNX individual address that will be used as source address for all non-raw frames.
  ...
  """
  @spec send_frame(KNXex.Frame.t(), Keyword.t()) ::
          :ok | {:error, term()}
  def send_frame(frame, opts \\ [])

  def send_frame(%KNXex.Frame{body: body} = frame, opts)
      when is_struct(body) and is_list(opts) do
    Protocol.assert_impl!(KNXex.Frames.FrameEncoder, frame.body.__struct__)
  ...
  def handle_call({:group_read, %KNXex.GroupAddress{} = group_address}, from, %State{} = state) do
    ...
    case known_gpa do
      ...
      _dpt ->
        frame = %KNXex.Frame{
          body: %KNXex.Frames.RoutingIndicationFrame{
            message_code: Constants.macro_assert_name(:message_code, :data_request),
            additional_info: "",
            payload: %KNXex.Frames.RoutingIndicationFrame.Data{
    ...
    frame_encoded =
      cond do
        is_struct(frame.body) ->
          KNXex.Frames.FrameEncoder.encode(frame.body, frame.protocol_version)
        ...
        request_type =
          case frame.request_type do
            :auto when is_struct(frame.body) ->
              Constants.by_name(
                :request_type,
                KNXex.Frames.FrameEncoder.get_request_type(frame.body)
              )
  ...
  # See the KNXex.FrameDecoder module
  @spec decode_frame(bitstring()) ::
          {:ok, KNXex.Frame.t()} | {:error, term()} | :ignore | :invalid
  defp decode_frame(
    ...
    case KNXex.FrameDecoder.decode_frame(
  ...
  defp handle_frame(
         %KNXex.Frame{
           ...
           body: %KNXex.Frames.RoutingIndicationFrame{
             ...
             payload:
               %KNXex.Frames.RoutingIndicationFrame.Data{
                 apci: apci
               } = body
           }
         } = frame,
    ...
    grpaddr_str = KNXex.GroupAddress.to_string(body.destination)
      ...
      datapoint_type ->
        ...
        dp_value =
          if apci == Constants.macro_assert_name(:frame_apci, :group_read) do
            ...
            KNXex.DPT.decode(body.value, datapoint_type)
          end
  ...
  defp call_frame_callback(%KNXex.Frame{} = frame, %State{} = state, type) do
  ...
  defp extract_opts(opts) when is_list(opts) do
    ...
    case opts[:source_address] do
      %KNXex.IndividualAddress{} = _map -> :ok
