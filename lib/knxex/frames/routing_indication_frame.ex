defmodule KNXex.Frames.RoutingIndicationFrame do
  @moduledoc """
  ...
  The payload has the following type for a specific message code:
    - `:data_request` or `:data_indicator` -> `KNXex.Frames.RoutingIndicationFrame.Data`
    - Any other -> `KNXex.Frames.RoutingIndicationFrame.Raw`
  """
  ...
  defmacro __using__(_any) do
    quote location: :keep do
      require KNXex.CEMIControlField
  ...
  defimpl KNXex.Frames.FrameEncoder do
    ...
    def encode(%{} = frame, Constants.macro_by_name(:knx, :protocol_version_10))
        when is_struct(frame, KNXex.Frames.RoutingIndicationFrame) do
    ...
    def encode(%{} = frame, Constants.macro_by_name(:knx, :protocol_version_10))
        when is_struct(frame, KNXex.Frames.RoutingIndicationFrame.Raw) do
      add_info_length = byte_size(frame.additional_info)
    ...
    defp do_encode_data(%KNXex.Frames.RoutingIndicationFrame.Data{} = frame) do
      {control_field, dest_addr} =
        case frame.destination_type do
          :group ->
            if not is_struct(frame.destination, KNXex.GroupAddress) do
              raise ArgumentError,
            ...
            {Bitwise.bor(frame.control_field, 0x80),
             KNXex.GroupAddress.to_raw_address(frame.destination)}

          :individual ->
            if not is_struct(frame.destination, KNXex.IndividualAddress) do
              raise ArgumentError,
            ...
            {Bitwise.band(frame.control_field, Bitwise.bnot(0x80)),
             KNXex.IndividualAddress.to_raw_address(frame.destination)}
        end
      ...
      tpci =
        if is_integer(frame.tpci) do
          ...
          KNXex.TPCI.to_integer(frame.tpci)
        end
      ...
      <<control_field::size(16), KNXex.IndividualAddress.to_raw_address(frame.source)::size(16),
        dest_addr::size(16), data_length::size(8), tpci_data::bitstring>>
    end

    defp do_encode_data(%KNXex.Frames.RoutingIndicationFrame.Raw{} = frame) do
      frame.raw_data
    end
  end
end
