defmodule KNXex.Frames.SearchResponseFrame do
  @moduledoc """
  ...
  defmacro __using__(_any) do
    quote location: :keep do
      def decode_frame(
        ...
        {:ok,
         %KNXex.Frames.SearchResponseFrame{
  ...
  defimpl KNXex.Frames.FrameEncoder do
    ...
    def encode(%{} = frame, Constants.macro_by_name(:knx, :protocol_version_10))
        when is_struct(frame, KNXex.Frames.SearchResponseFrame) do
      struct =
    ...
    @spec encode_dib(KNXex.Frames.SearchResponseFrame.t()) :: binary()
    defp encode_dib(%KNXex.Frames.SearchResponseFrame{dibs: dibs} = _frame) do
      Enum.reduce(dibs, <<>>, fn
