if Code.ensure_loaded?(KNXnetIP.Tunnel) do
  defmodule KNXexIP.TunnelClient do
    @moduledoc """
    ...
    """
    @spec send_telegram(KNXexIP.Telegram.t(), Keyword.t()) ::
            :ok | {:error, term()}
    def send_telegram(%KNXexIP.Telegram{} = telegram, opts \\ []) when is_list(opts) do
      validate_opts_arg(opts)
    ...
    def handle_call(
          {:send_telegram, %KNXexIP.Telegram{} = telegram},
          _from,
          %State{} = state
        ) do
      grpaddr_str = KNXexIP.GroupAddress.to_string(telegram.destination)
      ...
      if datapoint_type != nil or state.opts.allow_unknown_gpa do
        encode =
          case datapoint_type do
            ...
            _dpt -> KNXexIP.DPT.encode(telegram.value, datapoint_type)
          end
    ...
    def handle_call(
          {:group_read, %KNXexIP.GroupAddress{} = group_address},
          _from,
          %State{} = state
        ) do
      ...
      new_state =
        case state.current_telegram do
          {_cur_telegram, _ref} ->
            ...
            # spawn(fn ->
            #   {:ok, srcaddr} = KNXexIP.IndividualAddress.from_string(cur_telegram.source)
            #   {:ok, grpaddr} = KNXexIP.GroupAddress.from_string(cur_telegram.destination)

            #   my_telegram = %KNXexIP.Telegram{
      ...
      new_state =
        if datapoint_type != nil or state.opts.allow_unknown_gpa do
          ...
          decode =
            case datapoint_type do
              nil -> {:ok, telegram.value}
              _dpt -> KNXexIP.DPT.decode(telegram.value, datapoint_type)
            end
          ...
          case decode do
            {:ok, value} ->
              ...
              # to not block the Tunnelling ACK for too long
              spawn(fn ->
                {:ok, srcaddr} = KNXexIP.IndividualAddress.from_string(telegram.source)
                {:ok, grpaddr} = KNXexIP.GroupAddress.from_string(telegram.destination)
