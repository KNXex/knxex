defmodule KNXexIP.Frames.SearchResponseFrame do
  @moduledoc """
  ...
  @typedoc """
  Represents a KNX Search Response frame.
  """
  @type t :: %__MODULE__{
  ...
  defmacro __using__(_any) do
    quote location: :keep do
      def decode_frame(
            ...
            Constants.macro_assert_name(:request_type, :search_response),
        ...
        {:ok,
         %KNXexIP.Frames.SearchResponseFrame{
  ...
  defimpl KNXexIP.Frames.FrameEncoder do
    ...
    def encode(%{} = frame, Constants.macro_by_name(:knx, :protocol_version_10))
        when is_struct(frame, KNXexIP.Frames.SearchResponseFrame) do
      struct =
    ...
    def get_request_type(_frame), do: Constants.macro_assert_name(:request_type, :search_response)

    @spec encode_dib(KNXexIP.Frames.SearchResponseFrame.t()) :: binary()
    defp encode_dib(%KNXexIP.Frames.SearchResponseFrame{dibs: dibs} = _frame) do
      Enum.reduce(dibs, <<>>, fn
